...
{
    if (!test_flag2 && mag.at<short>(i, j) > 0 && edge.at<uchar>(i, j) != 255 && dir.at<uchar>(i, j) == 0)
    {
        int x = i;
        int y = j + 1;
...
int rout(int i, int j, Mat mag, Mat &edge, Mat dir, link_pix* &pix_now, const link_seg* seg_now)
{
    ...
    while (mag.at<short>(i, j) > 0 && edge.at<uchar>(i, j) != 255)
    {
        ...
        if (!test_flag && k>1 && i == pix_now->prev->prev->x && j == pix_now->prev->prev->y)
        {
            pix_now = pix_now->prev;
            i = pix_now->x;
            j = pix_now->y;
            pix_now->next = NULL;
//            std::cout << int(edge.at<uchar>(i, j)) << std::endl;
    ...
    while (mag.at<short>(i, j) > 0 && edge.at<uchar>(i, j) != 255)
    {
        ...
        if (!test_flag && k>1 && i == pix_now->next->next->x && j == pix_now->next->next->y)
        {
            pix_now = pix_now->next;
            i = pix_now->x;
            j = pix_now->y;
